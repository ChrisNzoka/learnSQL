...
-- Thus, over() is another way to do Group BY for aggregated functions
...
-- Notably, using order by within the over() function, we'll get rolling total as we proceed down the table
-- Over() works for agregated columns
SELECT dem.employee_id, dem.first_name, gender, salary,
...
-- ROW_NUMBER numbers the rows using the condition provided by the window function
SELECT dem.employee_id, dem.first_name, gender, salary,
